  ...
  execute(client, message) {
    ...
    if (!voiceChannel)
      return message.reply(
        '❌ You must be in a voice channel before I can change the volume on the currently playing track!',
      );

    if (isNaN(args[0]))
      return message.reply(
        '❌ You must enter a valid number between 0-100 to change the volume!',
      );
